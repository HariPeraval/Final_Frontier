CREATE OR REPLACE PACKAGE BODY ZMS.ppf IS
...
---------------------------------------------------------------------------------
  ...
  g_log_fname VARCHAR2(50):='zms_pp_pick.log';
...
/*********************************************************************/
   ...
   PROCEDURE P_PPF_Vdate_To_Pick_Dow;
   FUNCTION f_ppf_get_pick_day(pm_division IN NUMBER,
                           pm_wh       IN NUMBER,
                           pm_dept     IN NUMBER,
                           pm_class    IN NUMBER,
                           pm_watch_indicator OUT NUMBER)
   RETURN BOOLEAN;
   FUNCTION f_ppf_get_watch_indicator(pm_division IN NUMBER,
                                  pm_wh       IN NUMBER,
                                  pm_dept     IN NUMBER,
                                  pm_class    IN NUMBER)
   RETURN NUMBER;
   PROCEDURE p_ppf_fix_nopick_sku_group;
/*********************************************************************/
...
/*********************************************************************/
/*********************************************************************/
/* LOCAL PROCEDURE: p_ppf_fix_nopick_sku_group                       */
...
/* IR     | Modification                                             */
/*********************************************************************/
PROCEDURE p_ppf_fix_nopick_sku_group IS
   l_repl_sku_group ZMS_PPF_NOPICK_REPORT.REPL_SKU_GROUP%TYPE;
   l_division DIVISION.DIVISION%TYPE;
   l_store STORE.STORE%TYPE;
   l_sku ITEM_MASTER.ITEM%TYPE;
   l_unit_cost ZMS_PPF_NOPICK_REPORT.UNIT_COST%TYPE;
   l_unit_retail ZMS_PPF_NOPICK_REPORT.UNIT_RETAIL%TYPE;
...
/*********************************************************************/
PROCEDURE p_ppf_scarce_resources_ctl(O_err_code OUT INTEGER) IS

   l_orig_wh    ZMS_PPF_DIV_EXTRACT.WH%TYPE;
   l_process_no ZMS_PPF_DIV_EXTRACT.ORD_QTY%TYPE;
   l_pick_wh    ZMS_PPF_DIV_EXTRACT.WH%TYPE;
   l_prev_wh    ZMS_PPF_DIV_EXTRACT.WH%TYPE;
...
BEGIN
   ...
   /******************************************************************/
     ...
     utl_file.put_line(g_log_fptr, (TO_CHAR(SYSDATE, 'hh24:mi:ss')||' - p_ppf_scarce_resources_ctl started:'));
    ...
    LOOP
       ...
       utl_file.put_line(g_log_fptr, (TO_CHAR(SYSDATE, 'hh24:mi:ss')||' - SCARCE_RESOURCES - ZMS_PPF_WH_CONTROL Pick Ctl: Orig_WH: '|| l_orig_wh ||' - Process_No '|| l_process_no ||' - Pick_WH '|| l_pick_wh ||' - Prev_WH '|| NVL(l_prev_wh,0)));
       utl_file.fflush(g_log_fptr);
       p_ppf_alloc_scarce_resources(l_orig_wh, l_process_no, l_pick_wh, l_prev_wh, O_err_code);
    ...
    END LOOP;
     ...
     utl_file.put_line(g_log_fptr, (TO_CHAR(SYSDATE, 'hh24:mi:ss')||' - p_ppf_scarce_resources_ctl done:'));
...
EXCEPTION
   ...
   WHEN SCARCE_RESOURCE_ERROR THEN
        O_error_message := SQLERRM || ' problem in p_ppf_scarce_resources_ctl';
        O_err_code := 20101;
...
END p_ppf_scarce_resources_ctl;
/*********************************************************************/
/* LOCAL PROCEDURE: p_ppf_alloc_scarce_resources                     */
...
/*********************************************************************/
PROCEDURE P_PPF_ALLOC_scarce_resources (l_ctl_orig_wh    IN NUMBER,
  ...
  CURSOR c_skus IS
  SELECT ppf.sku,
         SUM(ppf.ord_qty) req_qty,
         COUNT(*) tot_num,
         ppf.wh
    FROM ZMS_PPF_DIV_EXTRACT ppf
   WHERE ppf.wh      = l_ctl_pick_wh
     AND ppf.orig_wh = l_ctl_orig_wh
     AND ppf.repl_sku_group = -1
   GROUP BY ppf.SKU, ppf.WH;
...
--sa   SELECT NVL(a.stock_avail,0) , NVL(b.st_pack_size,1)
   SELECT NVL(a.stock_avail,0) , NVL(b.inner_pack_size,1)
     FROM zms_PPF_WIN_WH a, item_supp_country b
...
BEGIN
      ...
      ***************************************************************/
-- Akron
--     DELETE FROM ZMS_PPF_DIV_EXTRACT Zpf
--     WHERE EXISTS (SELECT SKU
--                     FROM ZMS_PPF_WIN_WH wwz
...
--     COMMIT;
     ...
     g_log_fptr := utl_file.fopen(g_log_fdir, g_log_fname, 'A');
     fptr := utl_file.fopen(g_log_fdir,'zms_pp_pick_scarce_sku.log','A');
      ...
      IF NVL(l_ctl_prev_wh,0) <> 0 THEN
         ...
         SELECT ppf.store, sku, ppf.req, po_line_nbr, po_type, division, orig_ord_qty - ord_qty ord_qty, store_priority, in_str_date,
                priority, repl_sku_group, watch_priority, dist_type, l_ctl_pick_wh, ppf.request_sku, stock_on_hand + ord_qty,
                stock_avail, move_order_id, alloc_id, distro_date, cust_name, orig_wh, orig_ord_qty - ord_qty orig_ord_qty,
                ppf.req, po_type, 0 pick_process_nbr
           FROM zms_ppf_div_extract ppf
          WHERE ppf.orig_ord_qty - ppf.ord_qty > 0
            AND ppf.orig_wh        = l_ctl_orig_wh
            AND ppf.wh             = l_ctl_prev_wh
            AND ppf.repl_sku_group = -1;
     ...
     UPDATE zms_ppf_div_extract ppf
        SET ppf.ord_qty          = 0,
            ppf.pick_process_nbr = 1
      WHERE ppf.wh              = l_ctl_pick_wh
        AND ppf.orig_wh         = l_ctl_orig_wh
        AND ppf.repl_sku_group  = -1
        AND ppf.wh NOT IN (SELECT DISTINCT wh FROM zms_ppf_pick_day_stores)
        AND ppf.wh NOT IN (SELECT DISTINCT wh FROM zms_zpf_sku_PICK_DAY_STORES) 
        ;

    -- Set ord_qty = 0 when the warehouse does not have inventory
--     MERGE INTO zms_ppf_div_extract ppf USING
--       (SELECT DISTINCT ppf.wh, ppf.orig_wh, ppf.sku
--           FROM zms_ppf_div_extract ppf, zms_ppf_win_wh zwh, item_supp_country isc
--          WHERE ppf.sku     = zwh.sku(+)
--            AND ppf.wh      = zwh.wh(+)
--            AND ppf.sku     = isc.item
--            AND ppf.wh      = l_ctl_pick_wh
--            AND ppf.orig_wh = l_ctl_orig_wh
--            AND ppf.repl_sku_group      = -1
--            AND NVL(zwh.stock_avail,0)  < NVL(isc.inner_pack_size,0)
--            AND isc.primary_supp_ind    = 'Y'
--            AND isc.primary_country_ind = 'Y') dt
--     ON(ppf.wh = dt.wh and ppf.orig_wh = dt.orig_wh and ppf.sku = dt.sku)
...
--sa        END IF;
      ...
      /***************************************************************/
        ...
        --Akron - Keep records for next CTL Loop
        -- DELETE FROM ZMS_PPF_DIV_EXTRACT
        --  WHERE SKU = l_sku
        --    AND WH  = l_wh;
        --Akron
        -- DELETE ZMS_PPF_NOPICK_REPORT
        --  WHERE SKU = l_sku
   ...
   if l_count != tot_stores then
     ...
     i := 0;
   utl_file.putf(fptr,'Record in zms_ppf_div_extract was deleted for sku %s %s\n',ch
...
EXCEPTION
   ...
   WHEN OTHERS THEN
        O_error_message := SQLERRM || ' problem in p_ppf_scarce_resources';
        O_err_code := 20102;
        utl_file.put_line(g_log_fptr, (O_error_message));
        utl_file.fflush(g_log_fptr);

END p_ppf_alloc_scarce_resources;
/*********************************************************************/
/* LOCAL FUNCTION: f_ppf_get_watch_indicator                         */
/* DESCRIPTION: Retrieves the watch indicator from the ppf_dcs_div   */
...
END f_ppf_get_watch_indicator;
/*********************************************************************/
/* LOCAL PROCEDURE: p_ppf_vdate_to_pick_dow                          */
...
/* ================================================================= */
/*********************************************************************/
PROCEDURE P_PPF_Vdate_To_Pick_Dow IS
BEGIN
   /***************************************************************/
   ...
   IF (m_day_for_pick = 0) THEN
      m_day_for_pick := 7;
   END IF;
END P_PPF_Vdate_To_Pick_Dow;
/*********************************************************************/
/* LOCAL FUNCTION: f_ppf_get_pick_day                                */
...
/*********************************************************************/
/*********************************************************************/
/* PUBLIC FUNCTION: f_ppf_get_store_group                            */
...
/* ==================================================================*/
/*********************************************************************/
FUNCTION F_PPF_Get_Store_Group(pm_dept IN NUMBER,
                           pm_class IN NUMBER,
...
*/
END F_PPF_Get_Store_Group;

/*********************************************************************/
/*********************************************************************/
PROCEDURE p_ppf_div_extract_ext (p_is_forecast IN BOOLEAN,
                                 O_err_code   OUT INTEGER) IS
...
BEGIN
   ...
   INSERT /*+ APPEND */ INTO zms_PPF_DIV_EXTRACT nologging
   (STORE,
    SKU,
    REQ,
    po_line_nbr,
   ...
   (SELECT --/*+ INDEX(isc PK_ITEM_SUPP_COUNTRY) INDEX(R PK_REQ)*/
       t.to_loc,         --rd.STORE,
--sa10 t.item,           --rd.sku,
       CASE WHEN tsf_type = 'MR'  THEN t.item
...
--sa10       -1, --rd.repl_sku_group,
       NVL(sid.item,-1), --rd.repl_sku_group,
       0 watch_priority,
       --'AP' dist_type,
       zdt.dist_type,
       SUBSTR(t.from_loc,1,4) wh,
       t.item request_sku,
       0 stock_on_hand,
       cust_name,
       substr(t.from_loc,1,4) orig_whwh,
       DECODE(t.ord_qty + (isc.inner_pack_size-mod(t.ord_qty,isc.inner_pack_size)-isc.inner_pack_size),
          0,isc.inner_pack_size,
  ...
  FROM (SELECT /*+ parallel(td,8) */
               th.tsf_no, td.item, th.from_loc, th.to_loc, th.tsf_type,
               ...
               get_vdate vdate
          FROM (SELECT h.tsf_no, h.from_loc, h.to_loc, SUBSTR(h.comment_desc,1,200) cust_name,
                       CASE WHEN UPPER(h.comment_desc) LIKE 'SO%' THEN 'SO'
         ...
         (SELECT DISTINCT item FROM zms_ppf_main_sub) sid,
           --WHERE get_vdate BETWEEN a.start_date and a.end_date) sid,
         v_item_master im, item_supp_country isc, zms_ZALE_DIST_TYPE zdt, zms_all_location zal
   WHERE t.item   = im.item
     ...
     AND zdt.dist_type = DECODE(t.tsf_type,'PL','AP','MR','PT','AP')
     AND isc.primary_supp_ind = 'Y'
     AND isc.primary_country_ind = 'Y'
     AND ((substr(t.from_loc,1,4), t.to_loc)  IN
                    (SELECT WH, STORE
                    FROM zms_ppf_store_override
                    WHERE INDICATOR = 'Y'
                    UNION ALL
                    SELECT DISTINCT WH, rr.STORE
                    FROM ZMS_ZPF_PICK_DAY_STORES rr)
                    --FROM rms_replenishment rr
                    --WHERE group_name IN (l_sg1,l_sg2,l_sg3,l_sg4,l_sg5,l_sg6,l_sg7,l_sg8))
                       OR im.item IN (SELECT sku FROM zms_ppf_sku_override
                                       WHERE INDICATOR = 'Y'
                                             UNION ALL
                                      SELECT DISTINCT item FROM zms_ppf_sku_override so, sub_items_detail sid1
                                       WHERE so.sku = sid1.sub_item
                                         AND INDICATOR = 'Y')
                       OR im.item IN (SELECT sku item FROM pid_sku_master psm
                    WHERE EXISTS (SELECT 'x' FROM zms_ppf_dept_override zdd
   ...
   (SELECT --/*+INDEX(R PK_REQ)*/
    ...
    FROM (SELECT --/*+ ORDERED INDEX(ad PK_ALLOC_DETAIL) */
            ah.alloc_no, ah.item, ah.wh, ad.to_loc, ad.qty_allocated, ad.qty_transferred, ad.qty_distro, get_vdate vdate
         ...
         v_item_master im, ZMS_ZALE_DIST_TYPE zdt
   ...
   --AND t.tsf_no > 0
   --AND im.item > 0
     AND ((substr(a.wh,1,4), a.to_loc)  IN
                    (SELECT WH, STORE
                    FROM zms_ppf_store_override
                    WHERE INDICATOR = 'Y'
                    UNION ALL
   ...
   COMMIT;
       ...
       INSERT INTO zms_ppf_div_extract_bk
          (store, sku, req, po_line_nbr, po_type, division, ord_qty, store_priority,
           ...
           process_type, process_date, orig_wh, orig_ord_qty, orig_req,
           orig_po_type, pick_process_nbr)
       SELECT store, sku, req, po_line_nbr, po_type, division, ord_qty, store_priority,
              in_str_date, priority, repl_sku_group, watch_priority, dist_type, wh,
              request_sku, stock_on_hand, move_order_id, stock_avail, distro_date, cust_name, alloc_id,
   ...
   utl_file.put_line(g_log_fptr, (TO_CHAR(SYSDATE, 'hh24:mi:ss')||' -  Backup Process_Type 4: '||SQL%ROWCOUNT||'  '));

   COMMIT;

   --DELETE FROM zms_ppf_div_extract ppf
   -- WHERE sku <> -1
   --   AND (sku,wh) IN (SELECT wh.sku, wh.wh
   --                      FROM zms_ppf_win_wh1 wh
   --                     WHERE ppf.sku = wh.sku
   --                       AND stock_avail = 0);

-- Akron
   DELETE FROM zms_ppf_div_extract ppf
    WHERE sku <> -1
      AND (sku,division) IN (SELECT sku, division FROM
                               (SELECT wh.sku, ppf.division, SUM(NVL(wh.stock_avail,0)) stock_avail
                                  FROM zms_ppf_win_wh1 wh, zms_ppf_div_extract ppf, zms_ppf_dcs_div dcs
                                 WHERE ppf.sku = wh.sku
                                   AND ppf.division = dcs.division
                                   AND (wh.wh       = dcs.wh or dcs.alt_pick = 'Y')
                                 GROUP BY wh.sku, ppf.division)
                             WHERE stock_avail = 0);

   utl_file.put_line(g_log_fptr, (TO_CHAR(SYSDATE, 'hh24:mi:ss')||' -    Remove Skus No Stock_Avail  END: '||SQL%ROWCOUNT||'  '));
   utl_file.fflush(g_log_fptr);
   utl_file.fclose(g_log_fptr);
   g_log_fptr := utl_file.fopen(g_log_fdir, g_log_fname, 'A');

--   Trouble Shooting
--   SELECT wh.sku, wh.wh, wh.stock_avail, ppf.* FROM zms_ppf_div_extract ppf, zms_ppf_win_wh1 wh
--    WHERE ppf.sku <> -1
--      AND wh.sku = ppf.sku
--      AND (ppf.sku,ppf.wh) IN (SELECT wh.sku, wh.wh
--                         FROM zms_ppf_win_wh1 wh
--                        WHERE ppf.sku = wh.sku
--                          AND stock_avail = 0)
--    ORDER BY wh.sku, wh.wh, ppf.store
--
--   SELECT wh.sku, wh.wh, wh.stock_avail FROM zms_ppf_win_wh1 wh WHERE sku = 15423544

   SELECT count(*) INTO l_pick_cnt FROM  zms_ppf_div_extract;
   utl_file.put_line(g_log_fptr, (TO_CHAR(SYSDATE, 'hh24:mi:ss')||' -    Records in zms_ppf_div_extract: '||l_pick_cnt||'  '));
...
-- Akron
   ...
   --          FROM
   --               (SELECT  /*+ parallel(wh,3) parallel(ppf,3) use_hash(z,sid) index(UK_SUB_ITEMS_DETAIL sid) */
   --                        z.repl_sku_group,
   --                        z.wh,
   --                        sum(NVL(wh.stock_avail,0)) stock_avail
   --                  FROM zms_ppf_div_extract z, zms_ppf_win_wh1 wh,
   ...
   --          WHERE ppf.repl_sku_group  = a.repl_sku_group);
          ...
          (SELECT a.repl_sku_group, a.division
             FROM
                  (SELECT  /*+ parallel(wh,3) parallel(ppf,3) use_hash(z,sid) index(UK_SUB_ITEMS_DETAIL sid) */
                           ...
                           z.division,
                           SUM(NVL(wh.stock_avail,0)) stock_avail
                     FROM zms_ppf_win_wh1 wh,  zms_ppf_dcs_div dcs, zms_ppf_main_sub zms,
                          (SELECT DISTINCT division, repl_sku_group, sku
                             FROM zms_ppf_div_extract
                            WHERE sku = -1) z
                    WHERE z.repl_sku_group = zms.item
                      AND zms.sub_item = wh.sku
                      AND z.division   = dcs.division
                      AND (wh.wh       = dcs.wh or dcs.alt_pick = 'Y')
...
--                           z.wh,
--                           sum(NVL(wh.stock_avail,0)) stock_avail
--                     FROM zms_ppf_div_extract z, zms_ppf_win_wh1 wh, zms_ppf_main_sub zms
--                    WHERE z.repl_sku_group = zms.item
--                      AND zms.sub_item = wh.sku
--                      AND z.sku        = -1
--                    GROUP BY z.repl_sku_group, z.wh
...
--                     FROM zms_ppf_win_wh1 wh,
--                          (SELECT DISTINCT sub_item FROM zms_ppf_main_sub
--                            WHERE item = '18296327'
--                                  UNION
...
--                    WHERE zms.sub_item = wh.sku
   ...
   dbms_stats.gather_table_stats(
        ownname          => 'ZMS',
        tabname          => 'zms_PPF_DIV_EXTRACT',
        estimate_percent => 99,
        method_opt       => 'for all columns size repeat',
        DEGREE           => 30
       );

   utl_file.put_line(g_log_fptr, (TO_CHAR(SYSDATE, 'hh24:mi:ss')||' -    ANALYZE         END: '));

   INSERT /*+ append */ into zms_ppf_div_extract_wrk nologging
     ...
     SELECT z.sku, z.store, GREATEST(NVL(ils.stock_on_hand,0),0) - GREATEST(NVL(ils.non_sellable_qty,0),0) + GREATEST(NVL(ils.in_transit_qty,0),0)
       FROM (SELECT DISTINCT sku, store, repl_sku_group FROM zms_PPF_DIV_EXTRACT) z,
            item_loc_soh ils
   ...
   utl_file.put_line(g_log_fptr, (TO_CHAR(SYSDATE, 'hh24:mi:ss')||' -    Insert Into zms_ppf_div_extract_wrk: '||SQL%ROWCOUNT||'  '));

   COMMIT;

   MERGE INTO zms_PPF_DIV_EXTRACT ppf USING
     (SELECT  wrk.sku, wrk.store, wrk.stock_on_hand
      FROM zms_PPF_DIV_EXTRACT_wrk wrk
     ) dt
   ON(ppf.sku   = dt.sku    AND
      ppf.store = dt.store)
   WHEN MATCHED THEN
   UPDATE
   SET ppf.stock_on_hand = NVL(dt.stock_on_hand,0);
   ...
   MERGE INTO zms_PPF_DIV_EXTRACT ppf USING
     (SELECT  /*+ parallel(ils,3) */ --parallel(z,3) use_hash(z,sid) index(UK_SUB_ITEMS_DETAIL sid) */
        ...
        FROM zms_PPF_DIV_EXTRACT z, item_loc_soh ils, zms_ppf_main_sub zms
       WHERE z.repl_sku_group = zms.item
         AND zms.sub_item     = ils.item
         AND z.store          = ils.loc
         AND z.sku            = -1
       GROUP BY z.req, z.repl_sku_group, z.store) dt
   ON(ppf.repl_sku_group = dt.repl_sku_group AND
      ppf.store = dt.store AND
      ppf.req   = dt.req)
   WHEN MATCHED THEN
   UPDATE
   SET ppf.stock_on_hand = NVL(dt.stock_on_hand,0);
   ...
   (SELECT  /*+ parallel(ils,3) */ --parallel(z,3) use_hash(ils, z) */
        z.req, z.repl_sku_group, z.store,
        SUM(GREATEST(NVL(ils.stock_on_hand,0),0) - GREATEST(NVL(ils.non_sellable_qty,0),0) + GREATEST(NVL(ils.in_transit_qty,0),0)) stock_on_hand
      FROM zms_PPF_DIV_EXTRACT z, item_loc_soh ils
   ...
   WHEN MATCHED THEN
   UPDATE
   SET ppf.stock_on_hand = NVL(ppf.stock_on_hand,0) + NVL(dt.stock_on_hand,0);
   ...
   MERGE INTO zms_ppf_div_extract ppf USING
    (SELECT zss.sku, ppf1.store,
            ppf1.req, zdt.pick_priority
       FROM zms_ppf_special_sku zss,
            zms_ppf_div_extract ppf1,
            zms_zale_dist_type zdt
      WHERE zss.sku = ppf1.request_sku
        AND ppf1.stock_on_hand < 0
        AND ppf1.dist_type = 'PL'
        AND ppf1.division = zdt.division
        AND zdt.dist_type = 'NT') dt
   ON(dt.sku   = ppf.request_sku and
      dt.store = ppf.store     and
      dt.req   = ppf.req)
   WHEN MATCHED THEN
   UPDATE
   SET ppf.ord_qty   = ppf.ord_qty + (ppf.stock_on_hand * -1),
       ppf.dist_type = 'NT',
       ppf.priority  = dt.pick_priority;
...
/*  IF (p_is_forecast) THEN
    INSERT INTO ZMS_PPF_DIV_EXTRACT
      ...
      AND (TO_NUMBER(rt.STORE)  IN
                    ...
                    FROM rms_replenishment rr
                    WHERE group_name IN (l_sg1,l_sg2,l_sg3,l_sg4,l_sg5,l_sg6,l_sg7,l_sg8))
   OR   w.item IN    (SELECT sku FROM zms_ppf_sku_override
                    WHERE INDICATOR = 'Y')
...
/*
      ...
      AND (TO_NUMBER(rt.STORE)  IN
                    ...
                    FROM rms_replenishment rr
                    WHERE group_name IN (l_sg1,l_sg2,l_sg3,l_sg4,l_sg5,l_sg6,l_sg7,l_sg8))
      OR  EXISTS    (SELECT 'x' FROM zms_ppf_dept_override zdd
...
*/--sa01_end
   ...
   /* insert into ZMS_PPF_NOPICK_REPORT only for pick extracts. For forecast*/
   /* No need to touch ZMS_PPF_NOPICK_REPORT table as per IR#1882           */
   /*********************************************************************/
   IF (NOT p_is_forecast) THEN

   INSERT INTO ZMS_PPF_NOPICK_REPORT
   (SKU,
      REPL_SKU_GROUP,
      REQ,
      STORE,
   ...
   DELETE FROM ZMS_PPF_DIV_EXTRACT zde
   WHERE EXISTS
      (SELECT  'x'
       FROM zms_ppf_store_override zso
   ...
   /*********************************************************************/
   /* Update ZMS_PPF_NOPICK_REPORT only for pick extracts. For forecast     */
   /* No need to touch ZMS_PPF_NOPICK_REPORT table as per IR#1882           */
   /*********************************************************************/
   IF (NOT p_is_forecast) THEN

   UPDATE ZMS_PPF_NOPICK_REPORT znr
   SET znr.pick_qty = 0,
   ...
   utl_file.put_line(g_log_fptr, (TO_CHAR(SYSDATE, 'hh24:mi:ss')||' -    Records in zms_ppf_div_extract: '||l_pick_cnt||'  '));
     ...
     MERGE INTO zms_ppf_div_extract ppf USING
         (SELECT z.store, z.request_sku, z.req, NVL(r.priority_code,99998) priority_code
            FROM zms_ppf_div_extract z, pid_sku_master psm,
                 (SELECT * FROM zms_ppf_store_pick_priority WHERE group_name LIKE 'PICK_DEPT_PRIORITY_%') r
     ...
     ON(ppf.store = dt.store and ppf.request_sku = dt.request_sku and ppf.req = dt.req)
     WHEN MATCHED THEN
     UPDATE
     SET ppf.store_priority = dt.priority_code;
   ...
   /*********************************************************************/
   /* Update ppf_nopick_report only for pick extracts. For forecast     */
   /* No need to touch ppf_nopick_report table as per IR#1882           */
   /*********************************************************************/
   IF (NOT p_is_forecast) THEN
...
EXCEPTION
   ...
   WHEN OTHERS THEN
        O_error_message := SQLERRM || ' problem in p_ppf_div_extract_ext';
        O_err_code := 20104;
        utl_file.put_line(g_log_fptr, (O_error_message));
        utl_file.fflush(g_log_fptr);
        utl_file.fclose(g_log_fptr);
...
/*********************************************************************/
PROCEDURE p_ppf_divisional_extract IS
BEGIN
   /******************************************************************/
   /* call a procedure to do the initial data load                   */
   /******************************************************************/
-- p_ppf_div_extract_init(FALSE);
   /******************************************************************/
   ...
   /******************************************************************/
-- p_ppf_process_overrides(FALSE);
   /******************************************************************/
   /* process requisition lines for scarce resources                 */
   /******************************************************************/
-- p_ppf_alloc_scarce_resources(1,2,3,4);
   /******************************************************************/
   /* commit so we free up rollback segs                             */
   /******************************************************************/
   COMMIT;
END p_ppf_divisional_extract;
/*********************************************************************/
/* PUBLIC PROCEDURE: p_ppf_process_overrides                         */
/* DESCRIPTION: Includes or excludes SKUs for which requisitions     */
/*    exist based on the following criteria:                         */
/* Include:                                                          */
/* 1. pick day for the division for the SKU(ppf_dcs_div)             */
...
/*    ppf_store_overide where indicator = 'N'.  This takes care of   */
/*    priority 1 above.                                              */
...
/* Kishore Kumar 07/01/1997                                          */
/*********************************************************************/
PROCEDURE p_ppf_process_overrides (p_is_forecast IN BOOLEAN,
                                   O_err_code   OUT INTEGER) IS
   /******************************************************************/
   ...
   l_reason_code           ZMS_PPF_NOPICK_REPORT.REASON_CODE%TYPE;
   l_wh                    WH.WH%TYPE;
   ...
   SELECT sum(row_count) INTO l_override_cnt
     FROM
      (SELECT count(*) row_count FROM zms_ppf_sku_override
        WHERE indicator = 'N'
         UNION ALL
      SELECT count(*) row_count FROM zms_ppf_store_override
        WHERE indicator = 'N'
         UNION ALL
       SELECT count(*) row_count FROM zms_ppf_dept_override
        WHERE indicator = 'N');
  ...
  /******************************************************************/
   ...
   /*FCHK5.SQL*/
    ...
    EXECUTE IMMEDIATE 'truncate table zms_ppf_div_overrides';

    INSERT /*+ APPEND */ INTO zms_ppf_div_overrides NOLOGGING
    SELECT * FROM (
      ...
      (SELECT * FROM zms_ppf_sku_override
        WHERE indicator = 'N') skuo,
      (SELECT * FROM zms_ppf_dept_override
      ...
      */
        -- END IF;
      /***************************************************************/
      /* Remove rows from zms_ppf_div_extract if row on              */
      /*    zms_ppf_div_override                                     */
      /***************************************************************/


      DELETE FROM ZMS_PPF_DIV_EXTRACT
       WHERE (division, store, request_sku, req, repl_sku_group) IN
             (SELECT division, store, sku, req, repl_sku_group FROM zms_ppf_div_overrides);
               ...
               SET (pick_qty, reason_code) =
                   (SELECT pick_qty,
                           CASE WHEN sku_override1      IS NOT NULL THEN 'XSKU'
                                ...
                                ELSE 'XOTHER'
                           END reason_code
                      FROM zms_ppf_div_overrides zdo
...
EXCEPTION
   ...
   WHEN OTHERS THEN
        O_error_message := SQLERRM || ' problem in p_ppf_process_overrides';
        O_err_code := 20105;
        utl_file.put_line(g_log_fptr, (O_error_message));
        utl_file.fflush(g_log_fptr);
        utl_file.fclose(g_log_fptr);
...
END p_ppf_process_overrides;
/********************************************************************/
/*** PUBLIC PROCEDURE: p_ppf_upd_nopick_report                      */
/**** Author: Kishore Kumar                                         */
/********************************************************************/
PROCEDURE p_ppf_upd_nopick_report IS

CURSOR c_get_ord_qty IS
SELECT znr.ROWID c_rowid, zde.ord_qty c_ordqty
  FROM ZMS_PPF_DIV_EXTRACT zde , ZMS_PPF_NOPICK_REPORT znr
...
SELECT zde.SKU c_sku ,
       znr.ROWID z_rowid
  FROM ZMS_PPF_DIV_EXTRACT zde,
       ZMS_PPF_NOPICK_REPORT znr
 WHERE zde.STORE = znr.STORE
   AND zde.REQ   = znr.REQ
   AND zde.repl_sku_group = znr.repl_sku_group
   AND znr.SKU   = -1;

l_sku    ZMS_PPF_DIV_EXTRACT.SKU%TYPE;
l_ordqty ZMS_PPF_NOPICK_REPORT.PICK_QTY%TYPE;
l_rowid  ROWID;
s_rowid  ROWID;
BEGIN
   /******************************************************************/
   /* Update the ppf_nopick_report table for sku groups. The value   */
   /* sku will be -1 for sku_groups. This will get sku from extract  */
   /* table ppf_div_extract. The sku for sku_groups in extract table */
   ...
   /******************************************************************/
    ...
    COMMIT;
    /******************************************************************/
    /* Get the pick_qty from ppf_div_extract i.e the qty available for*/
    ...
    COMMIT;
    /*************************************************************/
    /* Delete the repl_sku_group records from ZMS_PPF_NOPICK_REPORT  */
    /* Which were excluded from pick                             */
    /*************************************************************/
    DELETE FROM ZMS_PPF_NOPICK_REPORT
     WHERE repl_sku_group IN (
                         SELECT repl_sku_group
                           FROM ZMS_PPF_NOPICK_REPORT znr
                          WHERE znr.SKU = -1
...
END p_ppf_upd_nopick_report;
/*********************************************************************/
/* PUBLIC PROCEDURE: p_ppf_reduce_to_max_pick                        */
...
/*********************************************************************/
PROCEDURE p_ppf_reduce_to_max_pick (pm_max_pick IN NUMBER,
                                pm_wh       IN NUMBER) IS
...
--sa   l_req            REQ.REQ%TYPE;
   l_sku            item_master.item%TYPE;
   l_store          STORE.STORE%TYPE;
   l_repl_sku_group ZMS_PPF_DIV_EXTRACT.repl_sku_group%TYPE;
   l_pick_qty       ZMS_PPF_NOPICK_REPORT.pick_qty%TYPE;
...
BEGIN
   ...
   /******************************************************************/
   IF pm_wh = 8450 THEN
    FOR l_req_records IN c_req_records_8450
    LOOP
     p_ppf_reduce_to_max_by_wh(pm_max_pick,
                          l_req_records.REQ,
                          l_req_records.SKU,
                          l_req_records.STORE,
   ...
   ELSE
    ...
    END LOOP;
   END IF;
END p_ppf_reduce_to_max_pick;
----------------------------------------------------------------------
/***************************************************************/
PROCEDURE p_ppf_reduce_to_max_by_wh (pm_max_pick IN NUMBER,
                                 pm_req      IN NUMBER,
                                 pm_sku      IN NUMBER,
                                 pm_store    IN NUMBER,
...
--      l_current_pick   := pm_current_pick;
      ...
      IF (pm_current_pick = pm_max_pick) THEN
         ...
         ELSE
            ...
            OPEN c_ppf_nopick_report;
            FETCH c_ppf_nopick_report
            INTO l_pick_qty, l_nopick_rowid;
            IF c_ppf_nopick_report%FOUND THEN
...
PROCEDURE P_PPF_EXCEEDED_MAX_PICK IS
   l_req_qty        NUMBER;
...
--sa   l_req            REQ.REQ%TYPE;
   ...
   /******************************************************************/
   /* retrieves the quantity requested from the ZMS_PPF_DIV_EXTRACT      */
   /* table                                                          */
   /******************************************************************/
...
g_log_fptr := utl_file.fopen(g_log_fdir, g_log_fname, 'A');
   ...
   LOOP
     ...
     IF (l_max_pick_qty < NVL(l_req_qty,0)) THEN
       -- P_PPF_REDUCE_TO_MAX_PICK(l_max_pick_qty, l_wh);
   ...
   /******************************************************************/
      IF l_wh = 8450 THEN
       FOR l_req_records IN c_req_records_8450
       LOOP
        p_ppf_reduce_to_max_by_wh(l_max_pick_qty,
                          l_req_records.REQ,
                          l_req_records.SKU,
                          l_req_records.STORE,
                          l_req_records.repl_sku_group,
...
END P_PPF_EXCEEDED_MAX_PICK;
/**************************************************************************/
/* PUBLIC PROCEDURE : p_ppf_alloc_sku_groups
  ...
  -- 05/20/2026 - Hari  - Added wh list from zms_zpf_sku_PICK_DAY_STORES
/**************************************************************************/
PROCEDURE p_ppf_alloc_sku_groups (p_is_forecast IN BOOLEAN,
                                  O_err_code   OUT INTEGER) IS
   l_req               ALLOC_HEADER.ALLOC_NO%TYPE;
--   l_req               REQDETAIL.REQ%TYPE;
   l_req               ZMS_PPF_DIV_EXTRACT.REQ%TYPE;
   l_sku               ZMS_PPF_DIV_EXTRACT.SKU%TYPE;
   l_division          ZMS_PPF_DIV_EXTRACT.DIVISION%TYPE;
   l_wh                WH.WH%TYPE;
   l_nopick_sku        ZMS_PPF_NOPICK_REPORT.SKU%TYPE;
   l_nopick_retail     ZMS_PPF_NOPICK_REPORT.UNIT_RETAIL%TYPE;
   l_nopick_cost       ZMS_PPF_NOPICK_REPORT.UNIT_COST%TYPE;
   l_stock_avail       ZMS_PPF_WIN_WH.STOCK_AVAIL%TYPE;
   l_repl_orig_wh      WH.WH%TYPE;
   l_repl_sku_group    ZMS_PPF_DIV_EXTRACT.REPL_SKU_GROUP%TYPE;
   l_ord_qty           ZMS_PPF_DIV_EXTRACT.ORD_QTY%TYPE;
...
-- l_ppf_req           ZMS_ZALE_FEEDBACK.REQ%TYPE;
...
-- l_zf_repl_sku_group ZMS_ZALE_FEEDBACK.REPL_SKU_GROUP%TYPE;
   l_current_pick_qty  ZMS_PPF_NOPICK_REPORT.PICK_QTY%TYPE;
   l_store             ZMS_PPF_DIV_EXTRACT.STORE%TYPE;
...
for each replenishment sku group */
      ...
      SELECT wc.process_no, ppf.orig_wh, ppf.repl_sku_group, SUM(NVL(ppf.ord_qty,0)) req_qty
        FROM zms_ppf_div_extract ppf, zms_ppf_wh_control wc
       WHERE ppf.sku     = -1
         AND ppf.orig_wh = wc.orig_wh
         AND ppf.orig_wh = wc.pick_wh
       GROUP BY wc.process_no, ppf.orig_wh, ppf.repl_sku_group
       ORDER BY wc.process_no, ppf.orig_wh, ppf.repl_sku_group;
...
/*The following cursor is used to determine the total stock at sku level
    for each replenishment sku group */
     ...
     SELECT wwz.sku,
      ...
      FROM ZMS_PPF_WIN_WH wwz, ITEM_SUPP_COUNTRY isc,
           zms_ppf_wh_control zwc, zms_ppf_main_sub zms
--sa      ZALE_REPL_SKU_GROUP_DETAIL@over_to_rtkp zrsgd
      WHERE zwc.orig_wh = l_repl_orig_wh --IN (8904,8591) -- 8450
        ...
        AND zwc.pick_wh IN (SELECT wh FROM zms_ppf_pick_day_stores union SELECT DISTINCT wh FROM zms_zpf_sku_PICK_DAY_STORES)
   ...
   CURSOR c_get_ppf_div_extract_repl IS
...
--     AND z.STORE          = sih.location
   ...
   ORDER BY z.priority,
      ...
      ZMS_PPF_DIV_EXTRACT.ORD_QTY%TYPE
      INDEX BY BINARY_INTEGER;

      TYPE st_store IS TABLE OF
        ZMS_PPF_DIV_EXTRACT.STORE%TYPE
      INDEX BY BINARY_INTEGER;

      TYPE st_req IS TABLE OF
        ZMS_PPF_DIV_EXTRACT.REQ%TYPE
      INDEX BY BINARY_INTEGER;

      TYPE st_po_line_nbr IS TABLE OF
        ZMS_PPF_DIV_EXTRACT.PO_LINE_NBR%TYPE
      INDEX BY BINARY_INTEGER;

      TYPE st_po_type IS TABLE OF
        ZMS_PPF_DIV_EXTRACT.PO_TYPE%TYPE
      INDEX BY BINARY_INTEGER;

      TYPE st_in_str_date IS TABLE OF
        ZMS_PPF_DIV_EXTRACT.IN_STR_DATE%TYPE
      INDEX BY BINARY_INTEGER;

      TYPE st_distro_date IS TABLE OF
        ZMS_PPF_DIV_EXTRACT.DISTRO_DATE%TYPE
      INDEX BY BINARY_INTEGER;

      TYPE st_cust_name IS TABLE OF
        ZMS_PPF_DIV_EXTRACT.CUST_NAME%TYPE
      INDEX BY BINARY_INTEGER;

      TYPE st_priority IS TABLE OF
        ZMS_PPF_DIV_EXTRACT.PRIORITY%TYPE
      INDEX BY BINARY_INTEGER;

      TYPE st_store_priority IS TABLE OF
        ZMS_PPF_DIV_EXTRACT.STORE_PRIORITY%TYPE
      INDEX BY BINARY_INTEGER;

      TYPE st_division IS TABLE OF
        ZMS_PPF_DIV_EXTRACT.DIVISION%TYPE
      INDEX BY BINARY_INTEGER;

      TYPE st_wh IS TABLE OF
        ZMS_PPF_DIV_EXTRACT.WH%TYPE
      INDEX BY BINARY_INTEGER;

      TYPE st_watch_priority IS TABLE OF
        ZMS_PPF_DIV_EXTRACT.WATCH_PRIORITY%TYPE
      INDEX BY BINARY_INTEGER;
...
g_log_fptr := utl_file.fopen(g_log_fdir, g_log_fname, 'A');
fptr := utl_file.fopen(g_log_fdir,'zms_pp_pick_sku_groups.log','W');

utl_file.put_line(g_log_fptr, (TO_CHAR(SYSDATE, 'hh24:mi:ss')||' - p_ppf_alloc_sku_groups started:'));
...
-- fptr := utl_file.fopen('ZSEND','zms_ppf_pick_scarce_sg','w');
   ...
   LOOP
    ...
    --utl_file.fflush(g_log_fptr);
    FOR l_get_ppf_div_extract_repl IN c_get_ppf_div_extract_repl
    LOOP
     ...
     mt_req(l_repl_cnt)         := l_get_ppf_div_extract_repl.REQ;
     mt_sku1(l_repl_cnt)        := l_get_ppf_div_extract_repl.SKU;
     mt_ord_qty(l_repl_cnt)     := l_get_ppf_div_extract_repl.ord_qty;
     mt_distro_qty(l_repl_cnt)  := 0;
     mt_repl_skgp1(l_repl_cnt)  := l_get_ppf_div_extract_repl.repl_sku_group;
     mt_store(l_repl_cnt)       := l_get_ppf_div_extract_repl.STORE;
     mt_po_line_nbr(l_repl_cnt) := l_get_ppf_div_extract_repl.po_line_nbr;
     mt_po_type(l_repl_cnt)     := l_get_ppf_div_extract_repl.po_type;
     mt_stock_on_hand(l_repl_cnt) := l_get_ppf_div_extract_repl.stock_on_hand;
     mt_dist_type(l_repl_cnt)   := l_get_ppf_div_extract_repl.dist_type;
     mt_division(l_repl_cnt)    := l_get_ppf_div_extract_repl.DIVISION;
     mt_wh1(l_repl_cnt)         := l_get_ppf_div_extract_repl.WH;
     mt_store_priority(l_repl_cnt)    := l_get_ppf_div_extract_repl.store_priority;
     mt_in_str_date(l_repl_cnt) := l_get_ppf_div_extract_repl.in_str_date;
     mt_distro_date(l_repl_cnt) := l_get_ppf_div_extract_repl.distro_date;
     mt_cust_name(l_repl_cnt)   := l_get_ppf_div_extract_repl.cust_name;
     mt_priority(l_repl_cnt)    := l_get_ppf_div_extract_repl.priority;
     mt_watch_priority(l_repl_cnt) := l_get_ppf_div_extract_repl.watch_priority;
     mt_fill_to_model(l_repl_cnt)  := l_get_ppf_div_extract_repl.ftm;
     mt_orig_wh(l_repl_cnt)        := l_get_ppf_div_extract_repl.orig_wh;
     mt_orig_req(l_repl_cnt)       := l_get_ppf_div_extract_repl.orig_req;
     mt_orig_po_type(l_repl_cnt)   := l_get_ppf_div_extract_repl.orig_po_type;
     mt_orig_ord_qty(l_repl_cnt)   := l_get_ppf_div_extract_repl.orig_ord_qty;
...
/****************************************************************************/
   ...
   /* Decrement the Stock in Sku from zms_ppf_win_wh */
  ...
  DELETE FROM ZMS_PPF_DIV_EXTRACT
     WHERE  SKU = -1;
  IF (NOT  p_is_forecast ) THEN
        /* Update pp_process_extract_check.SKU_GROUP_PROC to 'Y' if this procedure is
           finished successfully   added by Prasad */
...
/*      p_update_feedback_reqdetail;  */ /*will be called from proC*/
  ...
  utl_file.fflush(fptr);
  utl_file.fclose(fptr);
--utl_file.put_line(g_log_fptr, (TO_CHAR(SYSDATE, 'hh24:mi:ss')||' - ppf end:'));
...
EXCEPTION
   ...
   WHEN OTHERS THEN
        O_error_message := SQLERRM || ' problem in p_ppf_alloc_sku_groups';
        O_err_code := 20106;
        utl_file.put_line(g_log_fptr, O_error_message);
...
END p_ppf_alloc_sku_groups;

/***************************************************************************
The following procedure will update the feedback reqdetail tables
*****************************************************************************/
PROCEDURE p_ppf_upd_feedback_reqdetail(O_err_code OUT INTEGER) IS
...
g_log_fptr := utl_file.fopen(g_log_fdir, g_log_fname, 'A');
utl_file.put_line(g_log_fptr, (TO_CHAR(SYSDATE, 'hh24:mi:ss')||' - p_ppf_upd_feedback_reqdetail started:'));
...
/******************************************************************/
    ...
    -- Everything should be allocated by now
     ...
     EXECUTE IMMEDIATE 'truncate table zms_ppf_new_tsf_no drop storage';

     -- Assign new transfer numbers for records where orig_wh
     INSERT INTO zms_ppf_new_tsf_no
     ...
     MERGE INTO zms_ppf_div_extract ppf USING
        (SELECT a.wh, a.store, a.orig_wh, a.sku, a.po_type,
           ...
           FROM zms_ppf_div_extract a, zms_ppf_new_tsf_no b
          WHERE a.wh        = b.wh
            AND a.store     = b.store
            AND NVL(a.cust_name,' ') = NVL(b.cust_name,' ')
            AND a.orig_wh <> a.wh) dt
     ON(ppf.wh = dt.wh and ppf.store = dt.store and ppf.orig_wh = dt.orig_wh and
        ppf.sku = dt.sku and ppf.orig_req = dt.orig_req and NVL(ppf.cust_name,' ') = NVL(dt.cust_name,' '))
     WHEN MATCHED THEN
     UPDATE
     SET ppf.req = dt.req;

     utl_file.put_line(g_log_fptr, (TO_CHAR(SYSDATE, 'hh24:mi:ss')||' -  Assign New TSF Numbers to ppf_div_extract: '||SQL%ROWCOUNT));
     ...
     SELECT DISTINCT ppf.req tsf_no, NULL tsf_parent_no, 'W' from_loc_type, ppf.wh||'1001' from_loc,
         'S' to_loc_type, ppf.store to_loc, NULL exp_dc_date, NULL dept, 'A' inventory_type,
       ...
       FROM zms_ppf_div_extract ppf
      WHERE ppf.orig_wh <> ppf.wh
      ORDER BY ppf.wh||'1001', ppf.store;
     ...
     SELECT a.req tsf_no, (1000000 + rownum) tsf_seq_no, a.request_sku item, NULL inv_status, NULL tsf_price, a.ord_qty tsf_qty, 0 fill_qty,
       ...
       FROM (SELECT ppf.req, ppf.request_sku, SUM(ppf.ord_qty) ord_qty
               FROM zms_ppf_div_extract ppf
              WHERE ppf.orig_wh <> ppf.wh
              GROUP BY ppf.req, ppf.request_sku) a
...
--        (SELECT ppf.store, ppf.sku, SUM(NVL(ppf.ord_qty,0)) ord_qty
--           FROM zms_ppf_div_extract ppf
--          WHERE ppf.orig_wh <> ppf.wh
--          GROUP BY ppf.wh, ppf.sku) dt
...
--     utl_file.put_line(g_log_fptr, (TO_CHAR(SYSDATE, 'hh24:mi:ss')||' -  Update tsf_expected on item_loc_soh for Store: '||SQL%ROWCOUNT));
     ...
     MERGE INTO item_loc_soh ils USING
        (SELECT TO_NUMBER(ppf.wh||'1001') wh, ppf.request_sku, SUM(NVL(ppf.ord_qty,0)) ord_qty
           FROM zms_ppf_div_extract ppf
          WHERE ppf.orig_wh <> ppf.wh
          GROUP BY ppf.wh, ppf.request_sku) dt
     ...
     MERGE INTO item_loc_soh ils USING
        (SELECT TO_NUMBER(ppf.orig_wh||'1001') wh, ppf.request_sku, SUM(NVL(td.tsf_qty,0)) tsf_qty
           FROM zms_ppf_div_extract ppf, tsfdetail td
          WHERE ppf.orig_req    = td.tsf_no
            AND ppf.request_sku = td.item
            AND ppf.orig_wh    <> ppf.wh
          GROUP BY ppf.orig_wh, ppf.request_sku) dt
     ON(ils.loc = dt.wh and ils.item = dt.request_sku)
     WHEN MATCHED THEN
     UPDATE
     ...
     MERGE INTO tsfdetail td USING
        (SELECT ppf.orig_req, ppf.request_sku, SUM(NVL(ppf.ord_qty,0)) ord_qty
           FROM zms_ppf_div_extract ppf
          WHERE ppf.orig_wh  <> ppf.wh
            AND ppf.orig_req <> ppf.req
            AND ppf.po_type   = 'T'
          GROUP BY ppf.orig_req, ppf.request_sku) dt
     ...
     MERGE INTO alloc_detail ad USING
        (SELECT ppf.orig_req, ppf.store, SUM(NVL(ppf.ord_qty,0)) ord_qty
           FROM zms_ppf_div_extract ppf
          WHERE ppf.orig_wh     <> ppf.wh
            AND ppf.orig_req    <> ppf.req
            AND ppf.orig_po_type = 'A'
          GROUP BY ppf.orig_req, ppf.store) dt
     ...
     INSERT INTO zms_ifo_pick_div_extract
        ...
        SELECT ppf.sku, ppf.wh, ppf.store, ppf.po_type tsf_type, DECODE(ppf.dist_type,'SO','SO','RP') dist_type,
               SUM(ppf.ord_qty), ppf.req, zal2.store_div division, ppf.request_sku, ppf.distro_date, ppf.cust_name, 'RMS' data_source
          FROM zms_ppf_div_extract ppf, zms_all_location zal1, zms_all_location zal2
         WHERE ppf.wh           = zal1.loc_four_digit
           AND ppf.store        = zal2.loc
           AND zal1.akron_wh_ind = 'Y'
         GROUP BY ppf.sku, ppf.wh, ppf.store, ppf.po_type, DECODE(ppf.dist_type,'SO','SO','RP'),
                  ppf.req, zal2.store_div, ppf.request_sku, ppf.distro_date, ppf.cust_name;
...
/******************************************************************/
     ...
     MERGE INTO alloc_detail ad USING
     (SELECT ppf.req, ppf.store, ppf.po_type, SUM(ppf.ord_qty) ord_qty
        FROM zms_ppf_div_extract ppf
       WHERE ppf.po_type = 'A'
       GROUP BY ppf.req, ppf.store, ppf.po_type) dt
     ...
     MERGE INTO tsfdetail td USING
     (SELECT ppf.req, ppf.request_sku, SUM(ppf.ord_qty) ord_qty
       FROM zms_ppf_div_extract ppf
      WHERE ppf.po_type = 'T'
        AND ppf.wh = ppf.orig_wh
      GROUP BY ppf.req, ppf.request_sku) dt
     ...
     (SELECT DISTINCT ppf.req, ppf.po_type
        FROM zms_ppf_div_extract ppf
       WHERE ppf.po_type = 'T'
         AND ppf.wh = ppf.orig_wh) dt
     ON (th.tsf_no = dt.req)
     WHEN MATCHED THEN
     UPDATE
     SET th.status = 'L'
...
EXCEPTION
   ...
   WHEN OTHERS THEN
        O_error_message := SQLERRM || ' problem in p_ppf_upd_feedback_reqdetail';
        O_err_code := 20107;
        utl_file.put_line(g_log_fptr, (O_error_message));
        utl_file.fflush(g_log_fptr);
        utl_file.fclose(g_log_fptr);
...
END p_ppf_upd_feedback_reqdetail;
---------------------------------
PROCEDURE p_ppf_alloc_sku_groups_fcast IS
   l_stock_avail    ZMS_PPF_WIN_WH.stock_avail%TYPE;
   l_repl_sku_group ZMS_PPF_DIV_EXTRACT.repl_sku_group%TYPE;
   l_ord_qty        ZMS_PPF_DIV_EXTRACT.ord_qty%TYPE;
   l_pick_priority sub_items_detail.pick_priority%TYPE;
   ...
   TYPE t_sku IS TABLE OF
        ZMS_PPF_DIV_EXTRACT.SKU%TYPE
   INDEX BY BINARY_INTEGER;

   TYPE t_stock_avail IS TABLE OF
        ZMS_PPF_WIN_WH.stock_avail%TYPE
   INDEX BY BINARY_INTEGER;
   ...
   SELECT zms.sub_item SKU,
     ...
     FROM ZMS_PPF_WIN_WH wwz, zms_ppf_main_sub zms
    WHERE wwz.SKU = zms.sub_item
      AND wwz.WH IN (SELECT WH
                       FROM WH
                      WHERE vwh_type = 0 )  --wwz.WH = 8450
      AND zms.item = l_repl_sku_group
...
BEGIN
  ...
  *********************************************************************/
  ...
  IF  (l_stock_avail < l_ord_qty) THEN
   FOR l_div_rec IN c_get_ppf_div_extract_repl
   LOOP
 ...
 END LOOP;     /* l_get_repl */
END p_ppf_alloc_sku_groups_fcast;

-----------------------------------------------------------
-----------------------------------------------------------

PROCEDURE P_ppf_e3_EXCEEDED_MAX_PICK(return_code IN OUT VARCHAR2, O_error_message IN OUT VARCHAR2) IS
   ...
   l_store          STORE.STORE%TYPE;
   l_repl_sku_group ZMS_PPF_DIV_EXTRACT.repl_sku_group%TYPE;
   l_max_pick_qty   ZMS_PPF_NOPICK_REPORT.pick_qty%TYPE;
   l_nopick_rowid   ROWID;
...
BEGIN
   ...
   LOOP
     ...
     EXCEPTION
      WHEN OTHERS THEN
        O_error_message := SQLERRM || ' WHEN selecting FROM ZMS_PPF_DIV_EXTRACT TABLE';
        return_code := 'FALSE';
     END;
     IF (l_max_pick_qty < NVL(l_req_qty,0)) THEN
--        P_ppf_e3_REDUCE_TO_MAX_PICK(l_max_pick_qty, l_wh);
      ...
      IF l_wh = 8450 THEN
       FOR l_req_records IN c_req_records_8450
          LOOP
        p_ppf_e3_reduce_to_max_by_wh(l_max_pick_qty,
                          l_req_records.REQ,
                          l_req_records.SKU,
                          l_req_records.STORE,
                          l_req_records.repl_sku_group,
...
EXCEPTION
   ...
   WHEN OTHERS THEN
        O_error_message := SQLERRM || ' problem IN p_ppf_e3_exceeded_max_pick';
        return_code := 'FALSE';

END P_ppf_e3_EXCEEDED_MAX_PICK;

----------------------------------------------------------------------
/***************************************************************/
PROCEDURE p_ppf_e3_reduce_to_max_by_wh (pm_max_pick IN NUMBER,
                                 pm_req      IN NUMBER,
                                 pm_sku      IN NUMBER,
                                 pm_store    IN NUMBER,
                                 pm_repl_sku_group IN NUMBER,
...
END p_ppf_e3_reduce_to_max_by_wh;
